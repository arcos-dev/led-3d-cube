...
const ControlInput = ({ label, value, children }: ControlInputProps) => (
    <div className="mb-3">
        <label className="block text-sm font-medium text-gray-400 mb-1">{label} <span className="text-gray-200 font-mono">{value}</span></label>
        {children}
    </div>
);
  ...
  return (
    <div className="bg-[#161b22]/80 backdrop-blur-sm p-4 rounded-lg border border-[#30363d] shadow-lg w-full max-w-xs">
        ...
        <div className="flex items-center justify-between mb-3">
             ...
             <input type="checkbox" id="autoRotate" checked={autoRotate} onChange={(e) => onAutoRotateChange(e.target.checked)} className="form-checkbox h-5 w-5 text-gray-400 rounded bg-gray-700 border-gray-600 focus:ring-gray-500" />
        </div>
        ...
        <div className="flex items-center justify-between mb-4">
             <label htmlFor="showGrid" className="text-sm font-medium text-gray-400">Mostrar Estrutura</label>
             <input type="checkbox" id="showGrid" checked={showGrid} onChange={(e) => onShowGridChange(e.target.checked)} className="form-checkbox h-5 w-5 text-gray-400 rounded bg-gray-700 border-gray-600 focus:ring-gray-500" />
        </div>

        <button onClick={onResetCamera} className="w-full bg-gray-700 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded-lg transition-colors duration-200">
            Resetar Câmera
        </button>
    </div>
  );
